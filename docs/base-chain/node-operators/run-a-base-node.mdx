...
#### Base Peer Discovery (`basev0`)

Starting with v0.8.0, `base-reth-node` enables the Base-specific `basev0` discv5 peer identity by default via `--rollup.discovery.v5.base=true`. This significantly improves peer discovery on the Base network and resolves the low peer-count issues many operators experienced on earlier versions.
